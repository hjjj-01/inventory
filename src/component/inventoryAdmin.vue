<template>
    <div class="box">
        <div class="oprate-box">
            <div class="title">操作<span class="sector">多货登记后台系统</span></div>
            <div>
...
<script setup>
    ...
    const getStorageList = () => {//查询
        ...
        axios.post('http://134.175.18.3:8083/api/inventory/check',inputData.value[0],{
    ...
    const nextStep = () =>{//导出
        ...
        const date = new Date();
        const fileName = `多货库存数据_${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, '0')}${String(date.getDate()).padStart(2, '0')}.xlsx`;
    ...
    const delStorage = () =>{
        ...
        console.log(selectedRow.value)
        axios.post('http://134.175.18.3:8083/api/inventory/delete',
            selectedRow.value
        ,{
            headers: {'Content-Type': 'application/json'},
        })
