...
import axios from 'axios'
    ...
    const commit = () =>{
        localStorage.setItem('operator', commitData.value.operator)
        localStorage.setItem('stock', commitData.value.stock)
        ...
        axios.post('http://134.175.18.3:8083/api/inventory/commit', commitData.value, {headers: {'Content-Type': 'application/json'}})
        .then(res => {
